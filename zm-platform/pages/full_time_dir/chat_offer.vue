    ...
    <div class="full-time-container" v-if="permission">
        <el-form :inline="true" :model="search_data" class="search-form">
            ...
            <el-form-item label="职位类型">
                <el-select v-model="search_data.jobType" placeholder="请选择">
                    <el-option label="全部" value=""></el-option>
                    <el-option label="坐班" value="1"></el-option>
                    <el-option label="非坐班" value="2"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="进入掌门时间">
        ...
        <div class="context-body">
            ...
            <div class="table-head">
                <span class="table-title">共{{table_data_total}}个待沟通老师</span>
                <el-button size="small" @click="exportExcel" type="text" v-if="checkPermission('tea:process:export-excel')">导出Excel</el-button>
                <el-button size="small" @click="tableIconClick('batchStandUp','跑单')" type="text" :loading="tableIconClickLoading.batchStandUpLoad">跑单</el-button>
                <el-button size="small" @click="tableIconClick('trainingNotice','培训通知')" type="text" :loading="tableIconClickLoading.trainingNoticeLoad">培训通知</el-button>
            </div>
            ...
            <el-table :data="table_data" @selection-change="selectTable" style="width:100%">
                ...
                <el-table-column
                    v-if="checkPermission('tea:process:common-info')"
                    align="center"
                    label="姓名"
                    width="120"
                    fixed>
                    <template scope="scope">
                        ...
                        size="small"
                        type="text"
                        @click="userDetail(scope.row.id, scope.row.name, scope.row.mobile)">{{scope.row.name}}</el-button>
                    </template>
                </el-table-column>
        ...
        <el-dialog title="offer沟通的配置（修改后10s生效）" v-model="userDetailDialog" size="large" @close="closeUserDetailDialog">
            <div class="order_dialog_header">
                <div @click="offerIconClick" class="add-reserve">
                    <i class="el-icon-plus"></i>
                    <span>进入掌门时间</span>
                </div>
                <div @click="markIconClick" class="add-reserve">
                    <i class="el-icon-plus"></i>
                    <span>添加备注</span>
                </div>
            </div>
            <el-tabs v-model="userDetailTabNumber" @tab-click="userDetailTab" class="order_dialog_body">
                ...
                <el-tab-pane label="预约记录" name="third">
                    <el-table
                        :data="userDetailDataReserveRecords"
                        style="width: 100%">
                ...
                <el-tab-pane label="面试记录" name="fourth">
                    <div v-for="(item, index) in userDetailDataReviewRecords" :key="index" class="interview-result">
                        ...
                        <p>初步评价:
                            {{ item.firstEvaluateScoreStr }}
                        </p>
                        <p>
                            课件水平:
                                <el-rate
                                v-model="item.teachScore"
                                disabled
                                show-text
                                text-color="#ff9900"
                                text-template="{value}">
        ...
        <!--进入掌门时间弹窗-->
        <el-dialog title="进入掌门时间(10s后生效)" v-model="addOfferRecord" size="small" @close="closeOfferRecord">
            <el-form label-position="left" label-width="100px" class="add-review-form">
                <el-form-item label="进入掌门时间">
                    <el-date-picker
                         :editable="false"
                        v-model="addOfferData_enterTime"
                        type="date"
                        :picker-options="offerPickerOptions"
                        placeholder="选择日期">
                    </el-date-picker>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="addOfferRecord = false">取 消</el-button>
                <el-button type="primary" @click="addOffer" :loading="addOfferBlock">确 定</el-button>
            </div>
        </el-dialog>
        <!--添加备注弹窗-->
        <el-dialog title="添加备注" v-model="addMarkDialog" size="small" @close="closeAddMark">
            <el-form label-position="left" label-width="100px" class="add-review-form">
                <el-form-item label="备注">
                    <el-input
                    type="textarea"
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="addMarkDialog = false">取 消</el-button>
                <el-button type="primary" @click="addMark" :loading="addMarkBlock">确 定</el-button>
            </div>
        </el-dialog>
    </div>
    ...
    export default {
        data() {
            return {
                ...
                // 添加面试结果弹窗
                addOfferRecord:false,
                addOfferData_enterTime:'',
                addOfferBlock:false,
                commissioners:[],

                // table点击按钮
                tableIconClickLoading:{
                    batchStandUpLoad:false,
                    offerNoticeLoad:false,
                    trainingNoticeLoad:false,
                },
                ...
                offerPickerOptions: {
                    markDate(time) {
                        // return this.markList.indexOf(time) > -1;
                        return false;
                    }
                    // shortcuts: [{
                    //     text: '最近一周',
        ...
        methods: {
            ...
            // 选择日期范围
            chooseDate(){
                this.search_data.startEntryDate = new Date(this.searchDateArea[0]).getTime();
                this.search_data.endEntryDate = new Date(this.searchDateArea[1]).getTime();
            },
            // 查询
            search(){
                this.search_data.pageNo = 1;
            ...
            // 点击掌门时间按钮
            offerIconClick(){
                this.addOfferRecord=true;
                const dateTransform = this.$DateTool.format(new Date(),'yyyy-MM-dd');
                this.$Fetch({
                    url:'/api/offer//getTimeConflict',
                    method:'post',
                    data:{
                        entryDate:dateTransform,
                    ...
                    const message = result.message;
                    if (code ==0){
                        this.offerPickerOptions.markDate = function(time){
                            return data.indexOf(time)>-1;
                        }
                    } else {
                        this.$Message({
                            message
            ...
            // 进入掌门时间
            addOffer(){
                this.addOfferBlock = true;
                let time = new Date (this.addOfferData_enterTime)

                this.$Fetch({
                    url:'/api/offer/updateEntryDate',
                    method:'post',
                    data:{
                        entryDate:time.getFullYear()+"-"+(time.getMonth()<9?"0"+(time.getMonth()+1):time.getMonth()+1)+"-"+time.getDate(),
                        teacherId:this.userDetailId
                    }
                }).then((result)=>{
                    const data = result.data;
            ...
            addMark() {
                this.addMarkBlock = true;
                this.$Fetch({
                    url:'/api/offer/addTrainNoticeRemark',
                    method:'post',
                    data:{
                        id:this.userDetailId,
                        remark:this.addMarkData
                    }
                }).then((result)=>{
                    const data = result.data;
                    const code = result.code;
            ...
            // 导出excel
            exportExcel(){
                const url = `/api/teacher-web/api/offer/export?employmentNature=${this.search_data.employmentNature}&jobType=${this.search_data.jobType}`;
            ...
            tableIconClick(type,name){
                ...
                this.$Fetch({
                    ...
                    if (code ==0){
                        ...
                        let dataArray = data?Object.keys(data):[];
