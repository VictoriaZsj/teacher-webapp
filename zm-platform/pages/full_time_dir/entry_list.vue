    ...
    <div class="full-time-container" v-if="permission">
        <el-form :inline="true" :model="search_data" class="search-form">
            <el-form-item label="工作性质">
                <el-select v-model="search_data.teacherTimeType" placeholder="请选择">
        ...
        <div class="context-body">
            ...
            <div class="table-head">
                <span class="table-title">共{{table_data_total}}个入职老师</span>
            ...
            <el-table :data="table_data" @selection-change="selectTable" style="width:100%">
                ...
                <el-table-column prop="qq" align="center" label="QQ" width="125">
                </el-table-column>
                <el-table-column prop="college" align="center" label="毕业院校" width="200">
                </el-table-column>
                <el-table-column prop="hireTime" align="center" label="入职日期" width="200" :formatter="transformDate">
                </el-table-column>
                <el-table-column prop="salary" align="center" label="薪资" width="120">
                </el-table-column>
                <el-table-column prop="location" align="center" label="家庭住址" width="200">
                </el-table-column>
        ...
        <el-dialog title="入职名单的配置（修改后10s生效）" v-model="userDetailDialog" size="large" @close="closeUserDetailDialog">
            <div class="order_dialog_header">
                <div @click="addFarFarAwayDialog=true" class="add-reserve" v-if="checkPermission('tea:entried:far-far-away')">
        ...
        <!--离职弹窗-->
        <el-dialog title="离职(10s后生效)" v-model="addFarFarAwayDialog" size="small" @close="closeFarFarAwayDialog">
            <el-form label-position="left" label-width="120px" class="add-review-form">
                <el-form-item label="提交日期">
                    <el-date-picker :editable="false" v-model="leaveData.submitLeaveTime" type="date" placeholder="选择日期">
                    </el-date-picker>
                </el-form-item>
                <el-form-item label="离职日期">
                    <el-date-picker :editable="false" v-model="leaveData.leaveTime" type="date" placeholder="选择日期">
                    </el-date-picker>
                </el-form-item>
                <el-form-item label="离职原因">
                    <el-input v-model="leaveData.remark" :autosize="{ minRows: 4, maxRows: 6}" placeholder="请输入内容" type="textarea"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="addFarFarAwayDialog = false">取 消</el-button>
                <el-button type="primary" @click="addFarFarAway" :loading="addFarFarAwayBlock">确 定</el-button>
            </div>
        </el-dialog>
    </div>
...
export default {
    data() {
        return {
            ...
            // 离职弹窗
            addFarFarAwayDialog: false,
            // farFarAway: '',// 离职原因
            addFarFarAwayBlock: false,
            leaveData: {
                submitLeaveTime: '',
                leaveTime: '',
                remark: ''
    ...
    methods: {
        ...
        getTableData() {
            ...
            this.$Fetch({
                url: '/api/willHaveMoney/getWillHaveMoneyTeaNames',
                method: 'post',
        ...
        // 离职
        addFarFarAway() {
            this.addFarFarAwayBlock = true;

            this.$Fetch({
                url: "/api/willHaveMoney/farFarAway",
                method: 'post',
                data: {
                    id: this.userDetailId,
                    submitLeaveTime: new Date(this.leaveData.submitLeaveTime).getTime(),
                    leaveTime:new Date(this.leaveData.leaveTime).getTime(),
                    remark:this.leaveData.remark
                }
            }).then((result) => {
                const data = result.data;
                const code = result.code;
                ...
                if (code == 0) {
                    ...
                    this.addFarFarAwayDialog = false;
                    this.userDetailDialog = false;
                    this.getTableData();
                } else {
                    this.$Message({
        ...
        // 导出excel
        exportExcel() {
            const url = `/api/teacher-web/api/willHaveMoney/export?keyWord=${this.search_data.keyWord}&positionType=${this.search_data.positionType}&subject=${this.search_data.subject}&teacherTimeType=${this.search_data.teacherTimeType}`;
