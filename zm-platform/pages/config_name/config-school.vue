<template>
  <div class="config-school">
    <div class="tabs">
      <p :class="activeIndex === index ? 'active': ''" v-for="(item, index) in district" :key="index" @click="handleClick(index, item)">{{item.itemName}}</p>
    </div>
    <div class="school-container">
      <!-- <el-button type="primary" @click="handleShowDialog">新增学校</el-button> -->
      <div class="school-item">
        <div v-for="(item, index) in schoolItems" :key="index" :class="item.choose ? 'selected': ''" @click="handlerSelectSchool(index, item)">
          <span>{{item.lable}}</span>
          <!-- <i class="el-icon-circle-close" @click="handlerClickDele(index)"></i> -->
        </div>
      </div>
    </div>
    <el-dialog title="新增学校" :modal="modal" size="small" top="5%" v-model="addModel" class="parttime-form"
               @close="handlerCancle">
      <el-form ref="form" :model="schoolForm" label-width="80px" :label-position="labelPosition" size="small">
        <!-- <el-form-item label="所属地区">
          <el-select v-model="schoolForm.region" placeholder="请选择" style="width:190px;">
            <el-option :label="item.label" :value="item.value" v-for="(item, index) in tabs" :key="index"></el-option>
          </el-select>
        </el-form-item> -->
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      handleConfirm() {
        let option = {areaCode: this.areaCode, bu: this.bu ,lable: this.schoolForm.schoolName[0]}
