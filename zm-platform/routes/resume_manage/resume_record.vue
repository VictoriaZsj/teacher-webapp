<template>
    <div class="record-container">
        <el-form :model="ruleForm"  label-width='90px' ref="ruleForm"  inline class="demo-ruleForm" label-position="left">
            <el-row type="flex" class="row-bg" justify="center">
                <el-col :span="6">
                    <el-form-item label="院校等级" >
                        <el-select v-model="ruleForm.schoolLevels" class='selectHeight' multiple clearable placeholder="请选择">
                            <el-option
            ...
            <el-row type="flex" class="row-bg" justify="center">
                <el-col :span="6">
                    ...
                    <el-select v-model="ruleForm.certificateSituation" clearable style="min-height:33px">
                        <el-option
            ...
            <el-form-item label="投递邮箱"  >
                  <el-input clearable v-model="ruleForm.deliveryEmailAccount" placeholder="输入接收的邮箱"></el-input>
            </el-form-item>
            </el-col>
               <el-col >
                    <el-form-item label="年龄" >
                        ...
                        —
                        <el-input  v-model.number="ruleForm.age" style="width:90px" clearable ></el-input>
                    </el-form-item>
                </el-col>
                </el-row>
            <br>
             <el-row type="flex" class="row-bg" justify="center">
                <el-col :span="6">
                    <el-form-item label="老师" >
                        <el-input  v-model="ruleForm.teaNameOrMobile" placeholder="支持姓名/手机号查询" clearable></el-input>
                    </el-form-item>
                </el-col>
            ...
            <el-form-item label="失败/淘汰原因" >
                <el-input v-model="ruleForm.failedReason" clearable placeholder="失败/淘汰关键字查询"></el-input>
            </el-form-item>
            </el-col>
            </el-row>
            <br>
        ...
        <div>
            ...
            <el-button style="margin-left:20px;" size="small" type="text" @click="followObj.top=''">取消选择</el-button>
            <div style="float:right;">
                <el-button type="text"  @click='handleFollow_func' v-show="checkPermission('tea:resume:followUpBtn')">批量跟进</el-button>
                <el-button type="text"  @click='uploadDia=true'  v-show="checkPermission('tea:resume:uploadBtn')">导出</el-button>
                <el-button type="text"  @click='uploadListDia=true'  v-show="checkPermission('tea:resume:uploadBtn')">查看导出结果</el-button>
            </div>
        </div>
        ...
        ref="multipleTable"
            ...
            <el-table-column
                ...
                :label="item.label" :key='index' v-for="(item,index) in tableConfig" >
                <template scope="prop">
                    <el-button type="text" v-if="item.prop=='name'" size="small" @click="openImg(prop.row.ossAttachmentUrl)">{{prop.row.name}}</el-button>
            ...
            <el-table-column
                ...
                <template scope="scope">
                    <el-button @click="handleFollow(scope.row,'PASS','singlePass')" type="text" size="small" v-show="scope.row.followerResult!='PASS'">通过</el-button>
                    <el-button type="text" size="small" @click="handleFollow(scope.row,'ELIMINATE','singleEliminate')" v-show="scope.row.followerResult!='PASS'">淘汰</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
            ...
            <el-dialog :title="title" v-model="followObjVisible">
                <el-form :model="followObj" ref='diaForm' label-width="150px" :rules='rule'>
                    <el-form-item label="跟进结果：" prop='followResult' v-if="way!='singleEliminate'&&way!='singlePass'">
                        <el-radio-group v-model="followObj.followResult">
                            <el-radio label="PASS">通过</el-radio>
                            <el-radio label="ELIMINATE">淘汰</el-radio>
                        </el-radio-group>
                    </el-form-item>
                    <el-form-item label="指定推荐人："  required prop='commissionerUserId' v-if="way=='singlePass'||followObj.followResult=='PASS'" >
                     <el-select
                            style="width:40%"
                            v-model="followObj.commissionerUserId"
                            filterable
                            remote
                            clearable
                            reserve-keyword
                    ...
                    <el-form-item label=" 淘汰原因："  prop='failedReason' v-if="followObj.followResult=='ELIMINATE'">
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button @click="followObjVisible = false;$refs['multipleTable'].clearSelection()">取 消</el-button>
            ...
            <el-dialog
                ...
                width="30%"
                >
                <span>注意：由于数据量较大，所以下载速度会比较慢，点击【导出】后，系统会自动下载，下载完成后，在列表 页点击【查看导出结果】即可下载已导出的表格</span>
            ...
            <el-dialog
                title="导出列表"
                v-model="uploadListDia"
                width="30%"
                >
                <el-table
                    ref="multipleTable" :data="tableData_load"  border>
                        <el-table-column
                        align='center'
                        prop="createTime"
                        label="导出时间"
                        ...
                        width="100">
                        <template scope="scope">
                            <el-button @click="handleUpload(scope.row.exportFileUrl)" type="text" size="small" >下载</el-button>
                        </template>
                        </el-table-column>
                </el-table>
                  <el-pagination
...
import {options1,options2,options3,options4,options5,options6,options7,tableConfig} from './optionsConfig'
...
export default {
    ...
    watch:{
        followTime(val){
            ...
            //     this.followTime=[this.$DateTool.format(start, "yyyy-MM-dd") + " 00:00:00",this.$DateTool.format(end, "yyyy-MM-dd") + " 23:59:59"]
            // }
        },
        uploadListDia(val){
          if(val){
    ...
    methods:{
            ...
            this.$Fetch({
                ...
                if (code == 200&&data.records) {
                    ...
                    this.list = data.records.map(item => {
                        return { value: item.userId.toString(), label: item.teaName+'('+item.mobile+')'};
                    });
                    this.loading = false;
                } else {
                    this.list=[]
        ...
        getList(){
            ...
            this.ruleForm.deliveryStartTime=this.followTime[0]?this.followTime[0] +'00:00:00':'';
            this.ruleForm.deliveryEndTime=this.followTime[1]?this.followTime[1]+'23:59:59':'';

            this.$Fetch({
                url: '/api/zmbiz-clue-admin/api/resumeDelivery/getResumeDeliveryRecords',
                method: 'post',
                data: {
                    pageNo:this.pageNo,
