      ...
      'is-opened': opened
		...
		<template v-if="rootMenu.mode === 'horizontal' || (rootMenu.mode === 'vertical' && rootMenu.collapse)">
			<transition :name="menuTransitionName">
				<ul ref="sub" class="el-menu sub" v-show="opened">
					<slot></slot>
				</ul>
			</transition>
		</template>
...
export default {
  ...
  methods: {
    ...
    handleClick() {
      ...
      this.dispatch('ElMenuNew', 'submenu-click', this);
    },
    handleMouseenter() {
      const { rootMenu } = this;
      if (
    ...
    resizeSub() {
      if (this.$refs.li && this.$refs.sub) {
        let subHeight = Object.keys(this.items).length * 50;
        let allHeight = subHeight + this.$refs.li.offsetTop + 64;
        // let stop = document.documentElement.scrollTop;
        if (allHeight > window.innerHeight) {
          this.$refs.sub.style.top = window.innerHeight - 20 - allHeight + 'px';
        } else {
          this.$refs.sub.style.top = '0px';
        }
      }
    },
  },
